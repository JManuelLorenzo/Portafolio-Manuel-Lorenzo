...
// SEA MÁS APROPIADA
    ...
    stringBuilder.append(origen.getEtiqueta()).append(" "); // Agrega el origen
    Comparable anterior = origen.getEtiqueta();
    for (Comparable comparable : otrosVertices) {
        // Solo agrega si no es igual al anterior (evita duplicados consecutivos)
        if (!comparable.equals(anterior)) {
            stringBuilder.append(" ").append(comparable);
        }
        anterior = comparable;
    }
